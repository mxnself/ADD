...
class LabelSmoothedCrossEntropyCriterion(FairseqCriterion):
    ...
    def get_lprobs_and_target(self, model, net_output, sample):
        ...
        out= model.get_normalized_probs(net_output, log_probs=True, sample=sample)
        lprobs=out["prob"]
        tail_bias=out["tail_bias"]
        head_loss=out["head_loss"]
        ...
        return lprobs.view(-1, lprobs.size(-1)), target,head_loss

    def compute_loss(self, model, net_output, sample, reduce=True):
        lprobs, target,head_loss = self.get_lprobs_and_target(model, net_output, sample)
        kd_loss=None
        if model.kd_model is not None:
            kd_net_output=model.kd_model(**sample["net_input"])
            kd_lprobs,_,_=self.get_lprobs_and_target(model.kd_model, kd_net_output, sample)
            tmp_loss=kd_lprobs.exp() * (kd_lprobs - lprobs)
            if self.padding_idx is not None:
                pad_mask = target.unsqueeze(-1).eq(self.padding_idx)
                tmp_loss.masked_fill_(pad_mask, 0.0)
            kd_loss=tmp_loss.sum()
        ...
        return loss, nll_loss,kd_loss,head_loss
