...
class LabelSmoothedCrossEntropyCriterionWithCTC(
    ...
    def forward(self, model, sample, reduce=True):
        ...
        loss, nll_loss,kd_loss,head_loss = self.compute_loss(model, net_output, sample, reduce=reduce)

        # if kd_loss is not None:
        #     loss=loss+kd_loss*0.1
        ...
        if kd_loss is not None:
            logging_output["kd_loss"]=utils.item(kd_loss.data) if reduce else kd_loss.data
        if head_loss is not None:
            logging_output["head_loss"]=utils.item(head_loss.data) if reduce else head_loss.data
    ...
    def reduce_metrics(logging_outputs) -> None:
        ...
        kd_loss_sum = utils.item(
            sum(log.get("kd_loss", 0) for log in logging_outputs)
        )
        ...
        metrics.log_scalar(
            "head_loss", head_loss_sum / ntokens / math.log(2), ntokens, round=3
        )
        metrics.log_scalar(
