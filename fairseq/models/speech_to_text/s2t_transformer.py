...
class S2TTransformerModel(FairseqEncoderDecoderModel):
    ...
    def __init__(self, encoder, decoder,kd_model):
    ...
    def add_args(parser):
        ...
        # Average Decoder self-attn
        parser.add_argument(
            "--avg-decoder-selfattn",
            default=False,
            action='store_true',
            help="use average attn weight on decoder's self-attn",
        )

        # Average Decoder self-attn
        parser.add_argument(
            "--adaptive-softmax-gumbel",
            default=False,
            action='store_true',
            help="use gumbel-softmax on adaptive-softmax",
        )
        parser.add_argument(
            "--adaptive-softmax-head-loss",
            default=False,
            action='store_true',
            help="return adaptive-softmax's head loss",
        )
    ...
    def build_model(cls, args, task):
        ...
        if args.kd_model_path is not None:
            print("KD:{}".format(args.kd_model_path))
            state = checkpoint_utils.load_checkpoint_to_cpu(args.kd_model_path)
            model_args = state.get("cfg", None)
            task = tasks.setup_task(model_args.task)
            kd_model = task.build_model(model_args.model)
            kd_model.load_state_dict(state["model"], strict=True)
            for param in kd_model.parameters():
                param.requires_grad = False
        return cls(encoder, decoder, kd_model)
    ...
    def get_normalized_probs(
        ...
        out = self.get_normalized_probs_scriptable(net_output, log_probs, sample)
        out["prob"].batch_first = True
        return out
...
class TransformerDecoderScriptable(TransformerDecoder):
    ...
    def get_normalized_probs_scriptable(
        ...
        if hasattr(self, "adaptive_softmax") and self.adaptive_softmax is not None:
            ...
            if not log_probs:
                out["prob"]=out["prob"].exp_()

            return out

        logits = net_output[0]
        if log_probs:
            prob=utils.log_softmax(logits, dim=-1, onnx_trace=self.onnx_trace)
        else:
            prob=utils.softmax(logits, dim=-1, onnx_trace=self.onnx_trace)
        return {"prob":prob,"tail_bias":0}


@register_model_architecture(model_name="s2t_transformer", arch_name="s2t_transformer")
def base_architecture(args):
    # KD model
    args.kd_model_path = getattr(args, "kd_model_path", None)

    # Average Decoder Self-Attention
    args.avg_decoder_selfattn = getattr(args, "avg_decoder_selfattn", False)

    args.adaptive_softmax_gumbel = getattr(args, "adaptive_softmax_gumbel", False)
    args.adaptive_softmax_head_loss = getattr(args, "adaptive_softmax_head_loss", False)
    ...
    args.adaptive_softmax_type = getattr(args, "adaptive_softmax_type", "adaptive")
...
@register_model_architecture(model_name="s2t_transformer", arch_name="s2t_transformer_whisper")
def whisper_architecture(args):
    # KD model
    args.kd_model_path = getattr(args, "kd_model_path", None)
    ...
    # Conformer
    args.encoder_activation_fn = getattr(args, "encoder_activation_fn", "gelu")
    args.macaron_style = getattr(args, "macaron_style", False)
    args.use_cnn_module = getattr(args, "use_cnn_module", False)
    args.cnn_module_kernel = getattr(args, "cnn_module_kernel", 31)
